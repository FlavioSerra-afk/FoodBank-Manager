...
Single-template editor in admin (CodeMirror-backed) manages the registration markup with CF7-style tags; templates sanitized via a `wp_kses` allow-list that preserves fieldsets, headings, lists, and safe `aria-`/`data-` attributes while stripping scripts. Conditional visibility rules (phase-2) support grouped AND/OR logic with multiple conditions and actions per group (show, hide, require, optional); rules are stored with the template, progressively enhance the editor preview, and are re-validated on submission so hidden inputs are treated as absent and required/optional toggles are enforced server-side. Wave 6 adds JSON import/export with schema versioning, REST-backed preview/mapping, and a preset catalogue with placeholder prompts; the editor now debounces heavy recomputations, batches DOM writes, and exposes aria-live announcements plus keyboard reordering controls. Admin preview endpoint (`/fbm/v1/registration/preview`, manager-only nonce/cap) returns sanitized HTML, scrubbed warnings, and a transient nonce for an accessible modal preview (focus trap + ESC close) with an optional rule debugger.
...
Uploads respect WordPress MIME/size validation APIs with private attachment storage; multi-file payloads are rejected, filenames sanitized, and templates define min/max ranges (e.g. household size) that server-side handlers clamp before persistence.
...
Summaries (totals by date/week; active vs. revoked; collection counts). Member rows link to a detail view with fallback context and reverse-chronological attendance history.
