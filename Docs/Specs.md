...
Single-template editor in admin (CodeMirror-backed) manages the registration markup with CF7-style tags; templates sanitized via a `wp_kses` allow-list that preserves fieldsets, headings, lists, and safe `aria-`/`data-` attributes while stripping scripts. Conditional visibility rules (phase-2) support grouped AND/OR logic with multiple conditions and actions per group (show, hide, require, optional); rules are stored with the template, progressively enhance the editor preview, and are re-validated on submission so hidden inputs are treated as absent and required/optional toggles are enforced server-side. Admin preview endpoint (`/fbm/v1/registration/preview`, manager-only nonce/cap) returns sanitized HTML, scrubbed warnings, and a transient nonce for an accessible modal preview (focus trap + ESC close) with an optional rule debugger.
