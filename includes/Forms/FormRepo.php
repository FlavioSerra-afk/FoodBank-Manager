...
final class FormRepo {
    ...
    public static function create(array $data): int {
        $schema = self::normalize_schema($data['schema'] ?? [], (string)($data['title'] ?? 'Untitled'));
    ...
    public static function list(array $filters = []): array {
        ...
        $out = [];
        foreach ($posts as $p) {
            $out[] = ['id' => (int) $p->ID, 'title' => (string) $p->post_title];
        }
        return $out;
    }
    ...
    public static function normalize_schema(array $schema, string $title = 'Form'): array {
        $fields = [];
        $allowed = ['text', 'email', 'tel', 'date', 'select', 'checkbox', 'file'];
        $i = 1;
        foreach ($schema as $field) {
            ...
            $fields[] = [
                ...
                'options' => is_array($field['options'] ?? null) ? array_values(array_map('sanitize_text_field', $field['options'])) : [],
            ];
        }
