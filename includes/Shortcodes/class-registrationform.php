	...
	private static function current_template(): string {
        ...
        private static function handle_submission( array $fields, array $settings ): array {
                ...
                if ( $normalized['household_size'] < 1 ) {
                        $normalized['household_size'] = self::DEFAULT_HOUSEHOLD_SIZE;
                }
        ...
        private static function sanitize_field_value( array $definition, $raw ): array {
                ...
                switch ( $type ) {
                        ...
                        case 'textarea':
                                ...
                                break;
                        case 'number':
                                $value = is_string( $raw ) ? (int) $raw : ( is_numeric( $raw ) ? (int) $raw : null );
                                if ( null === $value ) {
                                        $value   = '';
                        ...
                        case 'household_size':
                                if ( is_numeric( $value ) ) {
                                        ...
                                        if ( isset( $range['min'] ) && is_numeric( $range['min'] ) ) {
                                                $size = max( $size, (int) $range['min'] );
                                        }

                                        if ( isset( $range['max'] ) && is_numeric( $range['max'] ) ) {
                                                $size = min( $size, (int) $range['max'] );
                                        }

                                        $normalized['household_size'] = $size;
                                }
                                break;
                        case 'consent':
                                if ( is_array( $value ) ) {
        ...
        private static function read_file_input( string $field ): ?array {
                if ( empty( $_FILES[ $field ] ) || ! is_array( $_FILES[ $field ] ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in Uploads::process().
                        return null;
                }

                $file = $_FILES[ $field ]; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in Uploads::process().
