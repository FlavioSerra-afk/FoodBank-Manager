(function () {
	function buildAttrs(tag){
		...
		if ( ! attrsWrap) {
			return;}
		attrsWrap.innerHTML = '';
		var sc              = (window.FBM_SHORTCODES || []).find(
			function (s) {
				return s.tag === tag;}
		);
		if ( ! sc) {
			return;}
		Object.entries( sc.atts ).forEach(
			function (entry) {
				var name          = entry[0];
				var info          = entry[1];
				...
				gen.addEventListener(
					...
					function () {
						...
						if ( ! tag) {
							return;}
						var parts = [];
						document.querySelectorAll( '#fbm-attrs [name^="atts["]' ).forEach(
							function (el) {
