...
function flattenPhpcs(array $json): array {
    $out = [];
    foreach (($json['files'] ?? []) as $file => $payload) {
...
function groupCount(array $rows, string $key): array {
    $c = [];
    foreach ($rows as $r) { $k = (string)($r[$key] ?? ''); $c[$k] = ($c[$k] ?? 0) + 1; }
    arsort($c);
    return $c;
}
...
function recipe(string $sniff): string {
    $map = [
        'WordPress.Security.EscapeOutput.OutputNotEscaped' =>
            "- **Fix:** Escape text with `esc_html()`, attributes with `esc_attr()`, URLs with `esc_url()`. For intentional HTML, sanitize with `wp_kses_post()` and echo with a one-line justified ignore only if PHPCS still complains.",
        'WordPress.Security.NonceVerification.Recommended' =>
            "- **Fix:** For POST/mutations add `wp_nonce_field()` + `check_admin_referer()`. For read-only GET filters sanitize and keep a one-line justified ignore.",
        'WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare' =>
            "- **Fix:** Build strict placeholders for `IN (...)` (e.g., `implode(',', array_fill(0, count(\$ids), '%d'))`), guard for empty arrays, and prepare values. Keep a one-line ignore only if PHPCS remains noisy.",
        'WordPress.Security.ValidatedSanitizedInput.InputNotSanitized' =>
            "- **Fix:** Wrap `\$_GET/\$_POST` with `wp_unslash()` + `sanitize_text_field()` / `absint()` / whitelist enums.",
        'WordPress.Security.ValidatedSanitizedInput.MissingUnslash' =>
            "- **Fix:** Use `wp_unslash()` before sanitizing superglobals.",
    ];
    foreach ($map as $k => $v) {
        if (str_starts_with($sniff, $k)) return $v;
    }
...
// Load ignores list
$ignores = file_exists(REPORT_IGN) ? file(REPORT_IGN, FILE_IGNORE_NEW_LINES) : [];
