...
function semverNext(string $v, string $mode): string {
    if (preg_match('/^\d+\.\d+\.\d+$/', $v) !== 1) throw new RuntimeException("Bad version: $v");
...
if (!$cnt1) { fwrite(STDERR, "Failed to update plugin header version\n"); exit(1); }
...
if (!$cnt2) { fwrite(STDERR, "Failed to update Plugin::FBM_VERSION\n"); exit(1); }
