...
final class UploadsTest extends TestCase {
	...
	public function test_process_rejects_multiple_files(): void {
			$file = array(
				'name'     => array( 'first.pdf', 'second.pdf' ),
				'type'     => array( 'application/pdf', 'application/pdf' ),
				'tmp_name' => array( '/tmp/first.pdf', '/tmp/second.pdf' ),
				'error'    => array( 0, 0 ),
				'size'     => array( 512, 1024 ),
			);
			...
			$this->assertSame( 'error', $result['status'] );
			$this->assertSame( 'Only one file may be uploaded.', $result['error'] );
	}
