...
final class ReportsRepositoryTest extends TestCase {
	...
	public function test_get_member_history_provides_fallback_member_context(): void {
			$repository = new ReportsRepository( new \wpdb() );

			$history = $repository->get_member_history( 'FBM-MISSING' );

			$this->assertIsArray( $history['member'] );
			$this->assertSame( 'FBM-MISSING', $history['member']['member_reference'] );
			$this->assertSame( array(), $history['rows'] );
	}
}
