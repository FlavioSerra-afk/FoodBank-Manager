...
final class ThemePageSaveTest extends \BaseTestCase {
    public function test_valid_theme_saved_once(): void {
        ...
        $opts = Options::sanitize_all($payload);
        update_option('fbm_options', $opts);
        $opts2 = Options::sanitize_all($payload);
        update_option('fbm_options', $opts2);
        $saved = Options::all();
        $this->assertSame('#123456', $saved['theme']['admin']['accent']);
    ...
    public function test_invalid_theme_rejected(): void {
        update_option('fbm_options', Options::defaults());
        $payload = array(
            'theme' => array('blob' => str_repeat('a', 70000)),
        );
        $opts = Options::sanitize_all($payload);
        $this->assertSame(Options::defaults()['theme'], $opts['theme']);
