...
final class AttendanceRepoTest extends TestCase {
    public function testPeopleSummaryExcludesVoidedByDefault(): void {
        ...
        $wpdb = new class {
            public $prefix = 'wp_';
            public $prepared = [];
            public function prepare($sql, $args) { $this->prepared[] = $sql; return $sql; }
            public function get_results($sql, $output) { return []; }
            public function get_var($sql) { return 0; }
        };
        AttendanceRepo::peopleSummary(['range_from'=>'2024-01-01 00:00:00','range_to'=>'2024-01-31 00:00:00']);
        $this->assertStringContainsString('t.is_void = 0', $wpdb->prepared[0]);
        $wpdb->prepared = [];
        AttendanceRepo::peopleSummary(['range_from'=>'2024-01-01 00:00:00','range_to'=>'2024-01-31 00:00:00','include_voided'=>true]);
